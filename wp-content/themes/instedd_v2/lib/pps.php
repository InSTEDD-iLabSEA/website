...
if (!PAGE_WIDGETS_SIDEBAR_CREATED) {
	register_sidebar(array(
	    ...
	    'name'=>'Technology Widgets',
	    'description'=>'Drag all widgets that you want to be visible on your technology pages. Once the widget is here you\'ll be able to choose it on individual page.',
...
function pps_print_the_box() {
    ...
    $checked = get_post_meta($_GET['post'], '_hide_all_widgets', true);
...
function pps_filter_frontend_widgets($widgets) {
    ...
    //     return $widgets;
    // }
    if ($post->post_type != 'page' || empty($widgets[PAGE_WIDGETS_SIDEBAR])) {
    	return $widgets;
    }
    
    $hide_all_widgets = get_post_meta($post->ID, '_hide_all_widgets', true);
    if ($hide_all_widgets) {
    	return array();
    }
