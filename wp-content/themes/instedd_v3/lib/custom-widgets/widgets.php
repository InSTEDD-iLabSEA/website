...
function widget_field_select_multiple($obj, $instance, $fieldname, $fieldtitle, $field_array) {
	$value = $instance[$fieldname];
	?>
	<p>
		<label for="<?php echo $obj->get_field_id($fieldname); ?>"><?php echo $fieldtitle; ?>:</label><br />
		<select name="<?php echo $obj->get_field_name($fieldname); ?>[]" id="<?php echo $obj->get_field_id($fieldname); ?>" style="width: 100%; height:auto; max-height:30em;" multiple="multiple">
			<?php foreach ($field_array['options'] as $val => $name) : ?>
				<option value="<?php echo $val; ?>" <?php echo (!(in_array($val, $value) === FALSE)) ? 'selected="selected"' : ''; ?>><?php echo $name; ?></option>
			<?php endforeach; ?>
		</select>
		<small>use &#8249;Ctrl&#8250; or Apple key to select multiples</small>
	</p>
	<?php
}
