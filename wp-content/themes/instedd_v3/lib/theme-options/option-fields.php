...
class wp_option_rich_text extends wp_option_textarea {
	function admin_init() {
		...
		wp_enqueue_script('tinymce_custom', get_option('home') . '/wp-includes/js/tinymce/tiny_mce.js');
		wp_enqueue_script('tinymce_custom_langs', get_option('home') . '/wp-includes/js/tinymce/langs/wp-langs-en.js');
		wp_enqueue_script(
			'tinymce_loader', 
			get_bloginfo('stylesheet_directory') . '/lib/theme-options/load-editor.js',
			array(),
			'0.1',
			true
        ); 
	    wp_enqueue_script( 
	    	'tinymce_lang',  
	    	get_option('home') . '/wp-includes/js/tinymce/langs/wp-langs-en.js' 
		);
		wp_option_textarea::admin_init();
	}
	
	function render() {
		$field_html = '<div id="editorcontainer"><textarea rows="10" mce_editable="true" class="theEditor" cols="40" name="' . $this->name . '">' . $this->value . '</textarea></div>';
	    return wp_option::render($field_html);
	}
}
