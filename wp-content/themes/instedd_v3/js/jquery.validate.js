...
$.extend($.validator, {
	...
	prototype: {
		...
		init: function() {
			...
			$(this.currentForm)
				.delegate("focusin focusout keyup", "input:text, input:password, input:file, select, textarea", delegate)
				.delegate("click", "input:radio, input:checkbox", delegate);
		...
		elements: function() {
			...
			.filter(":input")
			.not("input:submit, input:reset, input:image, [disabled]")
		...
		getLength: function(value, element) {
			...
			case 'input':
				if( this.checkable( element) )
					return this.findByName(element.name).filter('input:checked').length;
			}
			return value.length;
		},
